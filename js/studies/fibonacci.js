...
function duplicateCharacters(arr) {
  ...
  for(var i = 0; i < arr.length; i++){ dupeChars[String('_' + arr[i])] = 0; }

  //Increment the specific KEY/VALUE pair in the 'dupeChars' object
  for(var i = 0; i < arr.length; i++){ dupeChars[String('_' + arr[i])]++; }
  ...
  for(var key in dupeChars) {
    if(dupeChars[key] > 1) {
      duplicates[dupeCount] = key.slice(1); //slice off the _ from the key
      dupeCount++;
    }
  }
